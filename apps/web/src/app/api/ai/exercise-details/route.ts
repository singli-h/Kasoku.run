 ...
 * Request body: { sessionId: number, trainingGoals: string, exercises: Array<{presetId:number,name:string,type:string,existing:object}> }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { sessionId, trainingGoals, exercises } = body;
    ...
    const system = `You are an expert strength-and‐conditioning coach and a JSON-output specialist. Your job is to review a list of exercises in a training plan and automatically fill in missing “exercise details” for each, choosing only the minimal, most relevant variables per exercise and the user’s training goal. If an exercise already has some details, you can improve or refine them rather than overwrite them if needed. Every filled‐in exercise must include a very concise “reason” (≤ 30 words) explaining your choice.
...
6. If your first output is invalid JSON, correct it on the next turn, outputting only the corrected JSON.`;
    const userMessage = {
      role: 'user',
      content: JSON.stringify({ trainingGoals, exercises }, null, 2)
    };
