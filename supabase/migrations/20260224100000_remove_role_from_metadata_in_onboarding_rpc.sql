-- Migration: Remove redundant role write from metadata JSONB in complete_onboarding RPC
-- The users.role enum column is the single source of truth for role.
-- Writing role into metadata was a vestigial pattern that risked stale data.

-- Drop the old overload (without p_group_id) so CREATE OR REPLACE doesn't conflict
DROP FUNCTION IF EXISTS complete_onboarding(text, text, text, text, text, text, date, text, text, numeric, numeric, text, text, jsonb, jsonb, text, text, text);
...
BEGIN
  ...
  -- Prevent re-onboarding (guard against direct RPC calls)
  IF EXISTS (SELECT 1 FROM users WHERE clerk_id = p_clerk_id AND onboarding_completed = true) THEN
    RETURN QUERY SELECT false, NULL::INTEGER, 'Onboarding already completed'::TEXT;
    RETURN;
  END IF;
