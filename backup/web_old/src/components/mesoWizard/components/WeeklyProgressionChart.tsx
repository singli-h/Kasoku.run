...
interface WeeklyProgressionChartProps {
  data: WeeklyProgressionData[];
  onChange: (newData: WeeklyProgressionData[]) => void;
  modelType?: string | null;
}

const WeeklyProgressionChart: React.FC<WeeklyProgressionChartProps> = ({
  data = [],
  onChange = () => {},
  modelType = null,
}) => {
  const [localData, setLocalData] = useState<WeeklyProgressionData[]>(data);
  ...
  const getIntensityGradient = (): string => {
    return `linear-gradient(to right, hsla(25, 70%, 62%, 0.7), hsla(0, 90%, 40%, 0.9))`;
  };

  const getVolumeGradient = (): string => {
    return `linear-gradient(to right, hsla(210, 65%, 65%, 0.7), hsla(270, 85%, 40%, 0.9))`;
  };
            ...
            <table className="w-full border-collapse table-fixed">
              <thead>
                <tr className="border-b">
                  <th className="w-[8%] py-1.5 font-medium text-center text-xs sticky left-0 bg-white/80">
                    Week
                  </th>
                  {Array.from({ length: 10 }, (_, i) => (
                    <th
                      key={i}
                      className="w-[9.2%] py-1.5 font-medium text-center text-xs"
                    >
                      {i + 1}
                    </th>
                  ))}
              ...
              <tbody>
                {localData.map((weekData) => (
                  <tr
                    key={weekData.week}
                    className="border-b last:border-b-2 border-gray-200"
                  >
                    ...
                    <td colSpan={10} className="p-0">
                      <div className="grid grid-cols-10 h-8 sm:h-10">
                        {Array.from({ length: 10 }, (_, i) => (
                          <div
                            key={`intensity-${weekData.week}-${i}`}
                            className={`w-full h-full cursor-pointer transition-colors ${
                              i + 1 <= weekData.intensity
                                ? "text-gray-700"
                                : "text-gray-400"
                            }`}
                            style={{
                              backgroundColor: i + 1 <= weekData.intensity
                                  ? getIntensityColor(i + 1)
                                  : "transparent",
                              transition: "background-color 0.2s ease",
                            }}
                            onClick={() =>
                              handleCellClick(weekData.week, "intensity", i + 1)
                            }
                          />
                        ))}
                      </div>
                      <div className="grid grid-cols-10 h-8 sm:h-10 border-t border-gray-200">
                        {Array.from({ length: 10 }, (_, i) => (
                          <div
                            ...
                            style={{
                              backgroundColor: i + 1 <= weekData.volume
                                  ? getVolumeColor(i + 1)
                                  : "transparent",
                              transition: "background-color 0.2s ease",
                            }}
                            onClick={() =>
                              handleCellClick(weekData.week, "volume", i + 1)
                            }
                          />
                        ))}
                      </div>
