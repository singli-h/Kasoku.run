      ...
      try {
        ...
        const updatedSections = {
          warmup: presets
            ...
            .map((preset) => ({
              ...preset,
              exercise_type_id: exercises.find(
                (exercise) => exercise.id === preset.exercise_id
              )?.exercise_type_id,
            }))
  ...
  const handleSaveOrCreate = async (cloneCurrent = false) => {
    try {
      ...
      if (!groupResponse.ok) {
        throw new Error(
          isClone
            ? "Failed to clone preset group"
            : "Failed to save preset group"
        )
      }
      ...
      alert(
        isClone
          ? "Preset cloned and saved successfully!"
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
                            ...
                            >
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4 items-center">
                                {[
                                  ...
                                  ...(section === "gym"
                                    ? ["power", "velocity"]
                                    : []), // Include Power and Velocity only for gym
                                ].map((field) => (
                                  <div key={field} className="col-span-1">
                                    ...
                                    <input
                                      ...
                                      onChange={(e) => {
                                        const updatedSections = { ...sections }
                                        updatedSections[section][index][field] =
      ...
      <div className="flex grid-col-2 item-center justify-between">
        <Button onClick={() => handleSaveOrCreate(true)}>Create New</Button>
        {/* Only able to use save when preset selected */}
        {selectedGroup ? (
          <Button onClick={() => handleSaveOrCreate()}>Save Preset</Button>
        ) : (
          ""
        )}
      </div>
